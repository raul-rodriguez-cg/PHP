...
//        echo $consulta;
    ...
    public function guardar_pedido($referencia, $estado, $dir_recogida, $h_recogida, $dir_entrega,
        ...
        $sql = "INSERT INTO PEDIDOS(ESTADO_PEDIDO, DIR_RECOGIDA, H_RECOGIDA, DIR_ENTREGA, H_ENTREGA, T_ENTREGA, DISTANCIA, FK_ID_RIDER, FECHA_CREACION, REFERENCIA) 
                    VALUES('$estado', 
                           '$dir_recogida', 
                           " . ($h_recogida !== null ? "'$h_recogida'" : 'null') . ",
                           '$dir_entrega',
                           " . ($h_entrega !== null ? "'$h_entrega'" : 'null') . ",
                           " . ($tiempo !== null ? "'$tiempo'" : 'null') . ", 
                           null, 
                           " . ($id_rider !== null ? "$id_rider" : 'null') . ", 
                           NOW(), 
                           '$referencia')";
//        echo $sql;
        try{
            mysqli_query($conn, $sql);
            //echo'<script type="text/javascript"> alert("Pedido Guardado");window.location.href="'. $url. '";</script>';
            return;
        }catch(Exception $e){
            echo $e->getMessage();
            //echo'<script type="text/javascript"> alert("Error al guarrrdar");</script>';
            return;
        }finally{
            $conn->close();
        }
    ...
    public function editar_pedido($id_pedido, $referencia, $estado, $dir_recogida, $h_recogida, $dir_entrega,
        ...
        $sql = "UPDATE PEDIDOS SET 
                   ...
                   " . ($h_entrega !== null ? "'$h_entrega'" : null) . ",
                   T_ENTREGA = " . ($tiempo !== null ? "'$tiempo'" : null) . ",
                   DISTANCIA = null,
...
//       echo $sql;
        try{
            mysqli_query($conn, $sql);
            //echo'<script type="text/javascript"> alert("Pedido Editado");window.location.href="'. $url. '";</script>';
            return;
        }catch(Exception $e){
            echo'<script type="text/javascript"> alert("Error al editar");</script>';
            echo $e->getMessage();
            return;
        }finally{
            $conn->close();
    ...
    public static function cantidad_pedidos(): int{
        ...
        //TODO Creo que no es correcto asi
        $res = $res->fetch_assoc();
        //echo var_dump($res);
        return $res['TOTAL'];
    }
    ...
    public static function num_paginas_bueno(array $filtros){
        $conn = self::conn_sql();
        if ($conn->connect_error) {
            die("Error de conexión: " . $conn->connect_error);
        }
        ...
        //echo $consulta;
        $resultado =  mysqli_query($conn, $consulta)->fetch_assoc();
        return (isset($resultado['TOTAL'])) ? ceil($resultado['TOTAL']/self::$limite) : 0;
    }

    public function calcular_distancia(array $dist1,array $dist2):float|null{
    ...
    public function obtener_coordenada(String $direccion): array{
        ...
        if(empty($apiResult["data"][0]["latitude"]) || empty($apiResult["data"][0]["longitude"]))
        {
            return array();
        }
    ...
    public function guardar_distancias(float|null $distancia, int $id_pedido){
        try{
            $conn = self::conn_sql();
